...
### Регистр: PS не различает его нигде, Python различает везде

Самая дорогая ловушка портирования, потому что молчаливая. В PowerShell регистр не значим во
всём, куда попадает пользовательский ввод: свойства объекта из `ConvertFrom-Json`, ключи
`Hashtable`, `-eq`/`-contains`, имена параметров, `[ValidateSet]`. В Python совпадение точное,
поэтому `"CodeLength"` вместо `"codeLength"` в py-порте просто не находится: навык печатает
`[OK]`, а свойство в выход не попадает.

В py-портах ввод поэтому проходит через общие обёртки (реестр — в
`tests/skills/check-inline-drift.mjs`, семьи `case-insensitive input: *`):

```python
args = ci_parse_args(parser)              # имена параметров и значения choices — как ValidateSet
defn = ci_json(json.loads(json_text))     # словари DSL → CIDict: поиск без учёта регистра
enum_value_aliases = CIDict(enum_value_aliases)   # словари, по которым ищут ввод пользователя
```

`CIDict` хранит ключи КАК ЕСТЬ — часть из них это имена объектов, попадающие в XML;
регистронезависим только поиск. В `.ps1` этих обёрток нет и быть не должно.

Обёртка чинит поиск по словарю, но не сравнение строк: `if k in valid_types` в PS
регистронезависим (`-contains`), в Python нет. Такие места правятся точечно приведением к
одному регистру — и обязаны приводить значение к КАНОНУ списка, иначе прощённый регистр утечёт
в имя тега (в `meta-compile` `"type":"catalog"` так и делал `<catalog>`).
